...
exports.search=function(req, res) {
    ...
    var catId=req.query.cat;
    //前台传递的分页页码
    var page=parseInt(req.query.p,10)||0;//搜索框查询的时候没有传q默认为0
...
//通过搜索框执行的查找
        ...
        Movie
        .find({title:new RegExp(q+'.*','i')})//通过前台输入的q   查询标题为q的电影
        .exec(function(err,movies){
            ...
            //这才是分页的逻辑（前台传递的第几页index+后台规定的每页多少条）
            var results=movies.slice(index,index+count);//此情况默认index为0

            //只差徐一条数据
            res.render('results', {
                title: '结果列表页面',
                keyword:q,//关键词就是前台输入的查询条件
                currentPage:page+1,//当前页
                query:'q='+q,
                totalPage:Math.ceil(movies.length/2),//总页数
                movies: results
