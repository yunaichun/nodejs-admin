...
class Basic extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			tableProps: {
				...
				deleteCats: this.deleteCats.bind(this),
			},
			modalProps: {
				modalType: 'create',
				item: {}, //初始数据
	...
	componentDidMount() {
		this.props.selectCats().then(data => {
		...
		if (e.key === 'update') {
			...
			confirm({
				...
				onOk() { //确认删除
					...
					const id = text._id;
					that.props.deleteCat(id).then(data => {
						if (data.payload.status === '200') {
							that.setState({
								tableProps: Object.assign(
									{}, 
									that.state.tableProps, 
									{ data: that.props.catetoryReducer.data }
								)
							});
	...
	handleSave(addItem) {
		...
		that.props.insertCat(addItem).then(data => {
			if (data.payload.status === '200') {
				message.success(data.payload.msg, 2);
				that.setState({
					tableProps: Object.assign({}, that.state.tableProps, { data: that.props.catetoryReducer.data }),
					modalProps: Object.assign({}, that.state.modalProps,
						{ 
							visible: false,
							item: {}, //清空表单
	...
	handleUpdate(editItem) {
		...
		that.props.updateCat(editItem).then(data => {
			if (data.payload.status === '200') {
				//成功返回状态
				message.success(data.payload.msg, 2);
				that.setState({
	...
	deleteCats(params) {
		...
		this.props.deleteCats(ids).then(data => {
